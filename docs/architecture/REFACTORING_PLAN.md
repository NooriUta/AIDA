...
### C.1.0 Bug fixes ✅ DONE

> Все 7 оригинальных багов + 6 дополнительных фиксов применены.
> Полный отчёт: `HOUND_BUGFIX_REPORT.md`
...
## C.1.8 — BULK COLLECT lineage (✅ DONE — Apr 13, 2026)

**PR:** NooriUta/AIDA#4 · **Commit:** 99ede77 · **Branch:** claude/busy-johnson → master
**Scope:** 5 фич, 19 файлов, +966 строк

### Проблема
`DaliRecord` (BULK COLLECT collection variable) не был связан с INSERT-стейтментами через `RECORD_USED_IN`. В YGG было 0 записей несмотря на наличие `FORALL INSERT` паттернов в corpus.

### Реализовано

| Задача | Суть |
|---|---|
| **INFRA** `CorpusBatchIT` | Production corpus runner: 190 PL/SQL файлов, один writer + cleanAll() + CanonicalPool |
| **G6-EXT** FORALL INSERT → RECORD_USED_IN | Guard в `AtomProcessor` пропускал VALUES-атомы. Фикс: сканируем VALUES на `is_collection_field_access`, пишем в `StatementInfo.bulkCollectSources` |
| **G8** FETCH BULK COLLECT → RecordInfo | `enterInto_clause` игнорировал FETCH. Фикс: `exitFetch_statement` → `getCursorStmtGeoid()` → `ensureRecord()` |
| **DaliRecordField** vertex type | `field_geoid`, `field_name`, `field_order`, `record_geoid` + `HAS_RECORD_FIELD` edge + `FIELD_MAPS_TO` объявлен |
| **populateRecordFields** | Поля RecordInfo из columnsOutput курсорного SELECT (пропускает `*`) |

### Результаты corpus (190 файлов)

| Метрика | До | После |
|---|---|---|
| processed (без ошибок) | ~40 | 164 |
| DaliRecord | 0 | 54 ✅ |
| BULK_COLLECTS_INTO | 0 | 42 ✅ |
| RECORD_USED_IN | 0 | **3 ✅** |
| DaliStatement | — | 10 757 |
| DaliTable | — | 1 743 |
| DaliRecordField | — | pending (код в коммите, счётчик в следующем прогоне) |

### Беклог следующего Hound спринта

| # | Задача | Суть |
|---|---|---|
| G7 | SELECT * BULK COLLECT → поля из DDL | `columnsOutput = *` → нужен запрос `DaliColumn WHERE table_geoid=?` к ArcadeDB |
| G10 | FOR cur_rec IN cur LOOP → DaliRecord | Cursor loop variable не создаёт DaliRecord |
| G11 | FIELD_MAPS_TO: DaliRecordField → DaliColumn | Связь полей Record с колонками source-таблицы — основа DATA_FLOW через Record |
| G12 | findCursorSourceTable для JOIN-курсоров | Сейчас берётся только первая source table |
| G13 | Unit-тесты G6-EXT, G8, G10, G11 | Верификация без corpus run |
| **L2-DR** | **VERDANDI: отображение DaliRecord в LOOM** | Новый тип ноды в графе — отдельный стиль, drill-down, связи BULK_COLLECTS_INTO + RECORD_USED_IN + HAS_RECORD_FIELD видны в canvas |

### L2-DR — VERDANDI: DaliRecord node display

**Трек:** Track B (соавтор) · **Зависит от:** G11 FIELD_MAPS_TO (данные должны быть в YGG)

| Задача | Файл | Суть |
|---|---|---|
| L2-DR-1 | `src/types/lineage.ts` | Добавить тип `DaliRecord` в `NodeType` enum + интерфейс |
| L2-DR-2 | `src/components/nodes/RecordNode.tsx` | Кастомная нода: иконка буфера, поля Record как sub-items |
| L2-DR-3 | `src/utils/transformExplore.ts` | Маппинг DaliRecord → `RecordNode` при трансформации данных из YGG |
| L2-DR-4 | `src/styles/node-colors.ts` | Цвет DaliRecord: `#BA7517` (amber — временное хранилище) |
| L2-DR-5 | `src/components/panels/NodeInspector.tsx` | Drill-down панель: поля Record, source SELECT, target INSERT |

**Что должно быть видно в LOOM:**
```
[DaliTable: orders] ──READS_FROM──▶ [DaliStatement: SELECT]
                                            │
                                    BULK_COLLECTS_INTO
                                            ▼
                                    [DaliRecord: l_buffer]  ← новая нода
                                            │
                                      RECORD_USED_IN
                                            ▼
                                    [DaliStatement: FORALL INSERT]
                                     ...
                                     HAS_AFFECTED_COL
                                            ▼
                                    [DaliColumn: target.col]
```

**Приоритет:** Средний — нужно для demo (показывает сложный PL/SQL lineage), но не блокирует UC1.

**Ключевая lineage цепочка после G11:**
```
DaliTable(src).col
  → [FIELD_MAPS_TO]      → DaliRecordField
  → [HAS_RECORD_FIELD]   → DaliRecord
  → [BULK_COLLECTS_INTO] → DaliStatement(SELECT)
  → [RECORD_USED_IN]     → DaliStatement(INSERT)
...
| 12.04.2026 | 1.5 | **C.0.1 ArcadeDB 26.x DONE.** EmbeddedWriter удалён. Только REMOTE/REMOTE_BATCH. arcadedb-network 26.3.1. Тесты на REMOTE_BATCH. |
| 12.04.2026 | 1.4 | **C.1.8 BULK COLLECT lineage DONE** (PR #4, commit 99ede77). G6-EXT FORALL INSERT, G8 FETCH BULK COLLECT, DaliRecordField vertex type, populateRecordFields. Corpus: 164/190 files, DaliRecord 0→54, RECORD_USED_IN 3✅. Беклог G7/G10/G11/G12/G13. |
| 12.04.2026 | 1.3 | **C.1.0 DONE.** Все B1-B7 применены. Дополнительно: Fix1 orphaned DaliColumn (UniversalSemanticEngine), Fix2 double-schema geoid (StructureAndLineageBuilder), Fix3 HAS_COLUMN pool-cached (RemoteWriter), Fix4 UNBOUND false-positive (JsonlBatchBuilder), Fix5 schema cleanup (4 unused types), Fix6 TRUNCATE vs DELETE loop. Полный отчёт: HOUND_BUGFIX_REPORT.md. |
