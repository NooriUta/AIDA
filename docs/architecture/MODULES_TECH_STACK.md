...
- ⏳ Docker network `verdandi_net` → возможно переименовать в `aida_net` (отдельное решение)
...
| **Dali Core** | Backend / Orchestrator | **Java 21** | **Quarkus 3.34.2 + JobRunr (Open Source)** | — | 🔵 NEW |
| **MIMIR** | Backend / AI | TBD | Custom tool framework (возможен redesign через ArcadeDB MCP Server post-upgrade) | — | 🔵 NEW |
| **ANVIL backend** | Backend / Algorithms | TBD | Graph algorithm (возможно 72 built-in ArcadeDB algorithms post-upgrade) | — | 🔵 NEW |
| **HEIMDALL backend** | Cross-cutting | TBD (Quarkus proposed) | TBD | — | 🔵 NEW |
| **HEIMDALL frontend** | Frontend / Admin | TypeScript | React + same stack as VERDANDI | — | 🔵 NEW |
| **ANVIL UI** | Frontend (часть VERDANDI) | TypeScript | React (внутри VERDANDI) | — | 🔵 NEW |
| **FRIGG** | Data | — | **Отдельный ArcadeDB instance (unified SEER Studio state)** | — | 🔵 NEW |
...
Browser
    ↓  HTTPS
VERDANDI (React 19 SPA :13000 в dev, :15173 в prod)
    ↓  POST /graphql + Cookie sid=<uuid>
Chur (BFF/Gateway :3000) — auth, rate limit, CORS, GraphQL proxy
    ↓  POST /graphql + X-Seer-User + X-Seer-Role headers
SHUTTLE (Quarkus GraphQL API :8080) — 9 read queries + 2 KNOT queries
    ↓  HTTP REST
HoundArcade (ArcadeDB network :2480)
    ↑  ArcadeDB embedded engine (in-process write)
...
- **Hound пишет в YGG напрямую** (через embedded ArcadeDB engine), не через прокси. Dali Core НЕ является write proxy.
- **HoundArcade** — external ArcadeDB instance, доступен через сеть для SHUTTLE и других сервисов.
- **Keycloak** — single source of truth для auth, realm `seer`, client `aida-bff` (был `verdandi-bff`).
...
│   └── network: aida_net (rename from verdandi_net pending)
│
└── 🎨 Design system
...
|---|---|---|
| `/auth/login` | POST | Username/password → Keycloak Direct Access Grants → выдача sid cookie |
| `/auth/me` | GET | Текущий пользователь (auto-refresh если expired) |
| `/auth/refresh` | POST | Refresh access token |
| `/auth/logout` | POST | Удаление session + sid cookie |
| `/graphql` | POST | Proxy → SHUTTLE с trusted headers (X-Seer-User, X-Seer-Role) |
...
**Технологический стек (proposed):**

| Категория | Технология | Версия | Статус |
|---|---|---|---|
| Framework | React (тот же что VERDANDI) | 19.2.4 | 🔬 PROPOSED |
| Build | Vite | 8.x | 🔬 PROPOSED |
| Language | TypeScript | 5.9 | 🔬 PROPOSED |
...
| Testing | Vitest + Playwright | — | 🔬 PROPOSED |
| Port | 14000 (proposed, рядом с VERDANDI :13000) | — | 🔬 PROPOSED |

**НЕ берётся из VERDANDI** (не нужно для admin tool):
- React Flow (графа в HEIMDALL нет)
- ELK.js (layout не нужен)

**Repository:** новый отдельный git repo `heimdall` (не подкаталог VERDANDI)

**Shared code:** через npm package `aida-shared` (design tokens, auth client к Chur, общие types)

#### 3.6.2 HEIMDALL backend

**Тип:** Отдельный backend сервис (TBD язык/framework)

**Кандидаты для backend (из обсуждения):**

| Вариант | Описание | Pro | Con |
|---|---|---|---|
| **HB1** | Отдельный Quarkus сервис | Consistent с SHUTTLE/Hound, общий Gradle stack, native WebSocket через Vert.x, готовые SmallRye Reactive Messaging | +1 процесс в deployment |
| HB2 | Embedded в SHUTTLE как module | Не плодим процессы | SHUTTLE становится толстым, нарушает single responsibility |
| HB3 | Embedded в Dali (когда Dali на JVM) | Логично если Dali = orchestrator | Привязывает HEIMDALL к Dali, противоречит "admin всей AIDA" |
| HB4 | Отдельный Node.js сервис | Близко к BFF паттерну | Третий backend стек |

**Решение:** ⏳ PENDING (зависит от backend-стратегии — см. § 4)
**Текущая рекомендация:** HB1 (отдельный Quarkus сервис)

**Функции backend:**
- Event collector (приём events от Hound, Dali, MIMIR, ANVIL, SHUTTLE, VERDANDI, Chur)
- Ring buffer storage (10K events in-memory) — ADR-DA-003
- Metrics aggregation (counters, gauges, histograms, resolution tracker)
- WebSocket server для streaming в frontend
- Control command dispatch (reset, snapshot, replay → Dali)
- Demo recording / replay
- Health checks для всех AIDA services
- Audit log
...
**Dockerfiles:** все 4 сервиса (Hound, SHUTTLE, Chur, VERDANDI) имеют свой Dockerfile.

**Сетевая топология (порты):**

| Сервис | Dev port | Prod port | Описание |
|---|---|---|---|
| HoundArcade (ArcadeDB) | 18180 | 2480 | network mode |
| SHUTTLE | 18080 | 8080 | GraphQL API |
| Chur | 13000 | 3000 | BFF |
| VERDANDI | 15173 | 13000 | Vite dev / static |
| Keycloak | 8180 | 8180 | auth |
| HEIMDALL frontend | 14000 (proposed) | TBD | admin |

**Network:** `verdandi_net` → переименовать в `aida_net` (pending)
...
|---|---|---|
| **Java 21** | Hound, SHUTTLE | ✅ CONFIRMED |
| **TypeScript + Node.js 22** | Chur (BFF) | ✅ CONFIRMED |
| **TBD** | Dali Core, MIMIR, ANVIL backend, HEIMDALL backend | ⏳ PENDING (backend strategy) |
...
|---|---|---|---|
| ArcadeDB | YGG (HoundArcade) + FRIGG | 24.11.1 | ✅ CONFIRMED |
...
| Технология | Где используется | Версия | Статус |
|---|---|---|---|
| Quarkus | SHUTTLE | 3.34.2 | ✅ |
| SmallRye GraphQL | SHUTTLE | — | ✅ |
| Mutiny | SHUTTLE | — | ✅ |
| Fastify | Chur | 4.28.1 | ✅ |
| jose (JWT) | Chur | 6.2.2 | ✅ |
| Gradle | Hound + SHUTTLE | 9 | ✅ |
| ANTLR4 | Hound | 4.13.1 | ✅ |
...
|---|---|---|---|
| Q1 | **Backend стратегия** — JVM-first vs polyglot vs ad-hoc? | MIMIR, ANVIL backend, HEIMDALL backend, Chur migration | формально открыт, решения per-component |
| Q3 | **HEIMDALL backend** — HB1 (отдельный Quarkus) / HB2 (в SHUTTLE) / HB3 (в Dali) / HB4 (Node) | HEIMDALL engineering | mid May (после Q1). Sensible default = HB1 (Quarkus consistent с Dali/SHUTTLE) |
| Q4 | **HighLoad CFP deadline** — даты, slot, booth | весь roadmap | в течение 48 часов |

### 5.2 Важные

| # | Вопрос | Заметки |
|---|---|---|
| Q5 | **Co-founder split** — кто Track A (data/algorithms), кто Track B (UI/orchestration) | в течение недели |
| Q6 | **HEIMDALL event schema** — TypeScript canonical interface, payload schemas | mid May |
| Q7 | **HEIMDALL ↔ Dali control commands API** — формат, REST endpoints, или JobRunr API | end of April (после Q1, Q3) |
...
| Q18 | **Docker network rename** `verdandi_net` → `aida_net` | cosmetic, после major changes |
