...
**Документ:** `LOOM_PERFORMANCE_ANALYSIS`
**Дата:** 12.04.2026
**Статус:** Working document — mitigation план для реализации
...
| Файл | Что менять |
|---|---|
| `frontends/verdandi/src/utils/constants.ts` | `LARGE_GRAPH_THRESHOLD`, `TIMEOUT_MS`, добавить `AUTO_GRID_THRESHOLD = 800`, `DENSE_GRAPH_RATIO = 5` |
| `frontends/verdandi/src/utils/layoutGraph.ts` | M-1 дедупликация, M-2 алгоритм switch, M-3 авто-grid, M-6 rolling hash |
| `frontends/verdandi/src/hooks/canvas/useLoomLayout.ts` | M-3 уведомление пользователю, M-7 timeout |
| `frontends/verdandi/src/hooks/canvas/useDisplayGraph.ts` | M-5 авто tableLevelView при V>500 |
...
| Задачи | Когда |
|---|---|
| XS (M-1, M-3, M-5, M-6, M-7) — один день, Quick wins | До июля, пока LOOM не нагружен production данными |
| S (M-2) — auto-detect density + алгоритм switch | До August demo |
| L (M-4) — edge virtualization | Отдельный спринт, post-August |
